...
class Model(nn.Module):
  def __init__(self):
      ...
      self.base_model = nn.Sequential(*list(swin3d_s(pretrained=False).children())[:-1])
...
class VideoRecog_Model(nn.Module):
  def __init__(self):
      ...
      self.base_model = nn.Sequential(*list(swin3d_s(pretrained=True).children())[:-1])
      self.fc1 = nn.Linear(4, 51)
...
for epoch in range(1, total_epochs + 1):
    ...
    scheduler.step()
    with open("Swins_log.txt", "a") as myfile:
        myfile.write("Epoch ",epoch,",",train_loss,",",train_accuracy,",",val_loss,",",val_accuracy)
